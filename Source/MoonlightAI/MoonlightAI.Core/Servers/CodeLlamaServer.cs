...
public class CodeLlamaServer : IAIServer, IDisposable
{
    ...
    /// <param name="promptService">The prompt service for loading prompts.</param>
    public CodeLlamaServer(HttpClient httpClient, AIServerConfiguration configuration, PromptService promptService)
    {
    ...
    public Task<AIResponse> GenerateMethodXmlDocumentationAsync(string method, CancellationToken cancellationToken = default)
    {
        ...
        // Use PromptService to get the model-specific prompt
        var variables = new Dictionary<string, string>
        {
            ["method"] = method
        };

        var prompt = _promptService.GetPrompt("codedoc", "method", _configuration.ModelName, variables);
